...
function checkMissingIntents(client) {
    ...
    console.warn(
        `${chalk.gray(`[${new Date().toLocaleTimeString([], { hour12: true })}]`)}${chalk.yellow.bold(' ⚠ WARNING ')}${chalk.white(' │ ')}Missing gateway intents detected: ${chalk.yellow(missingIntentNames.join(', '))}`
      );
}
